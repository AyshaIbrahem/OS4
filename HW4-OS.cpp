...
struct pcb {
    string name;
    int arrival_time, burst_time, start_time, finish_time, remaining_time;
};
...
void context_switch(int &time, vector<string> &timeline) {
    // This simulates context switching time
...
vector<string> get_shortest_remaining_time_timeline(vector<pcb> processes) {
    ...
    for (int time = 0, index = 0; ; time++) {
        ...
        ready_queue[min_process].remaining_time--;
        Sleep(SLEEPING_INTERVAL);
        timeline.push_back(ready_queue[min_process].name);

        if (ready_queue[min_process].remaining_time == 0) {
...
void print_gantt_chart(vector<string> timeline) {
    ...
    for (int i = 0; i < timeline.size(); i++) {
        cout << i << "   " << timeline[i] << "   ";
        while (i + 1 < timeline.size() && timeline[i] == timeline[i + 1]) {
            i++;
        }
    }
    cout << endl;
...
void set_start_and_finish_time(vector<pcb> &processes, vector<string> timeline) {
    for (int i = 0; i < processes_count; i++) {
        for (int j = 0; j < timeline.size(); j++) {
            if (processes[i].name == timeline[j] && processes[i].start_time == 0) {
...
int main() {
    ...
    // Get timelines and calculate results for each scheduling algorithm
